...
import { ArrowRight, Star, TrendingUp, Package, Briefcase, Monitor, Printer, FileText } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow">
        ...
        <section className="py-16 bg-background">
          <div className="container mx-auto px-4">
            <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl font-bold mb-4 text-foreground">All Office Products</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto">Browse our complete office collection.</p>
            </motion.div>
            {officeProducts.length > 0 ? (
              <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}><ModernProductGrid products={officeProducts} /></motion.div>
            ) : (
              <div className="text-center py-12 bg-card rounded-2xl border border-border">
                <Briefcase className="w-16 h-16 mx-auto mb-4 text-muted-foreground" />
                <p className="text-muted-foreground mb-4">No office products available yet. Check back soon!</p>
                <Link to="/products"><Button className="bg-gradient-to-r from-blue-600 to-indigo-600">View All Products</Button></Link>
              </div>
            )}
          </div>
        </section>
